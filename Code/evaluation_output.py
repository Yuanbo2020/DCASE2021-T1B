...
class ImageClassify(object):
    ...
    def fusion_multimodel_topN(self):
        ...
        for fileid in list_chunk[target_chunk]:
            ...
            # print(np.max(imagedata), np.min(imagedata))
            # # 255 3
            image = imagedata.reshape((2240, 224, 3))
            imagedata = cv2.cvtColor(image, cv2.COLOR_BGR2RGB).reshape((10, 224, 224, 3))
            ...
            for each_model in final_modellist:
                ...
                print('num of paras:', para)
                print('Model {} : params: {}'.format(self.model._get_name(), para ))
                # 102284274
                # 28329010
                ...
                with torch.no_grad():
                    ...
                    top_y_possible.append(possible[0])
            ...
            # print(final_average_y_possible)
            # final_average_y_possible = np.round(final_average_y_possible, decimals=4)
            # print(final_average_y_possible)

            # prob = final_average_y_possible.tolist()
            es_class = np.argmax(final_average_y_possible)

            print(fileid, label_list[es_class], final_average_y_possible)

            data = list(final_average_y_possible)  # 将一组10个概率值放入列表
