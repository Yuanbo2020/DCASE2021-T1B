...
class Audio_Data(Dataset):
    def __init__(self,data_root="/home/share/tyz/dataset_2fps",type="train"):
        ...
        for i in fragments:
            ...
            path = os.path.join(data_root,"audio",class_id,fragment_id,fragment_id+".pickle")
            self.data.append([path, classes_dic[class_id], fragment_id, location_id])
    ...
    def __getitem__(self, idx):
        audio_add, class_id, fragment_id, location_id = self.data[idx]

        with open(audio_add, 'rb')as file:
            n = pickle.load(file)
        audio = torch.from_numpy(n)
        return audio, class_id, fragment_id, location_id
